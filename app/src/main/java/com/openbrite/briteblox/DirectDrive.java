...
public class DirectDrive extends Object {
    ...
    public static void setBaseContext(Context ctx_in) {
        ctx = ctx_in;
    }

    /**
     * Send one single message to the BriteBlox board.  Will be displayed as soon as the board is done displaying current messages.
     *
    ...
    /**
     * Send one single message to the BriteBlox board, with padding.
     * Will be displayed as soon as the board is done displaying current messages.
     *
     * @param message The text you wish to propagate to the matrix
     */
    ...
    protected static class driver extends AsyncTask<Void, Integer, Integer> {
        ...
        static String deviceType;               // what device the user is controlling -- BriteBlox, Wearable, or whatever else
        static int deviceColumns;               // number of LED columns on the specified device
        static int deviceRows;                  // number of LED rows on the specified device
        ...
        static int boards = 0;                  // number of LED display boards in the matrix
        static int boardHeight;                 // height (rows) in pixels of a single board
        static int boardWidth;                  // width (columns) in pixels of a single board
        ...
        protected void onPreExecute() {
            ...
            boards = Integer.parseInt(prefs.getString("marqueeLength", "2"));
            deviceType = prefs.getString("deviceType", "BriteBlox");
            boardHeight = MyApp.productRows.get(deviceType);
            boardWidth = MyApp.productColumns.get(deviceType);
            for (int i = 1; i < maxMsgs; i++)
                MyApp.messages[i] = "";
            if (deviceType.equals("BriteBlox")) {
        ...
        private void serialThread() {
            if (deviceType.equals("BriteBlox")) {
                while (true)  // mind you, this is the implementation coming straight from the firmware ;)
                {
                    ...
                    {
                        int delta = 1024 / (boards * 1);
                        // Standard arrangement -- one row worth of numBoards panels
                        eom[0] = (byte) (delta * i / 16);  // Each processor looks for its own special "code"
                ...
                char[] messageChars = MyApp.messages[0].toCharArray();
                byte[] messageBytes = new byte[messageChars.length];
                for (int i = 0; i < messageChars.length; i++) {
                    Log.d("BblxWearable", "Char " + i + " = " + ((int) messageChars[i]));
                    messageBytes[i] = (byte) messageChars[i];
                }
                MyApp.mLogService.write(messageBytes);
            }
